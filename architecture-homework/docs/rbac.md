...
- `roles` - RBAC ролі для роута, передаються у формі {global?: [`MEMBER`, `SYSADMIN`], tenant?: [`TEACHER`], event?: [`ORGANIZER`]}
...
ABAC у цій моделі використовується для контекстних обмежень (`isOwner`, `isAssignedTeacher`, `isSameTenant`).
...
router.patch(
  ...
  authorize({
    ...
    checks: [isSameTenant, isAssignedTeacher],
  }),
  handler,
);
```
